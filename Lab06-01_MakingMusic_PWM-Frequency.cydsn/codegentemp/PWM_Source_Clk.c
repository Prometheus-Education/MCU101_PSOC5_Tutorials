...
/*******************************************************************************
* Function Name: PWM_Source_Clk_Start
********************************************************************************
*
* Summary:
...
void PWM_Source_Clk_Start(void) 
{
    ...
    PWM_Source_Clk_CLKEN |= PWM_Source_Clk_CLKEN_MASK;
	PWM_Source_Clk_CLKSTBY |= PWM_Source_Clk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: PWM_Source_Clk_Stop
********************************************************************************
*
* Summary:
...
void PWM_Source_Clk_Stop(void) 
{
    ...
    PWM_Source_Clk_CLKEN &= (uint8)(~PWM_Source_Clk_CLKEN_MASK);
	PWM_Source_Clk_CLKSTBY &= (uint8)(~PWM_Source_Clk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: PWM_Source_Clk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((PWM_Source_Clk_CLKEN & PWM_Source_Clk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = PWM_Source_Clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = PWM_Source_Clk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* PWM_Source_Clk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(PWM_Source_Clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(PWM_Source_Clk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: PWM_Source_Clk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: PWM_Source_Clk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = PWM_Source_Clk_GetSourceRegister();
    uint16 oldDivider = PWM_Source_Clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = PWM_Source_Clk_CLKEN & PWM_Source_Clk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(PWM_Source_Clk_DIV_PTR, clkDivider);
                PWM_Source_Clk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                PWM_Source_Clk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(PWM_Source_Clk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((PWM_Source_Clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: PWM_Source_Clk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(PWM_Source_Clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: PWM_Source_Clk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    PWM_Source_Clk_MOD_SRC |= modeBitMask & (uint8)PWM_Source_Clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: PWM_Source_Clk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    PWM_Source_Clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(PWM_Source_Clk_MODE_MASK));
}


/*******************************************************************************
* Function Name: PWM_Source_Clk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return PWM_Source_Clk_MOD_SRC & (uint8)(PWM_Source_Clk_MODE_MASK);
}


/*******************************************************************************
* Function Name: PWM_Source_Clk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = PWM_Source_Clk_GetDividerRegister();
    uint8 oldSrc = PWM_Source_Clk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        PWM_Source_Clk_MOD_SRC |= CYCLK_SSS;
        PWM_Source_Clk_MOD_SRC =
            (PWM_Source_Clk_MOD_SRC & (uint8)(~PWM_Source_Clk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: PWM_Source_Clk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return PWM_Source_Clk_MOD_SRC & PWM_Source_Clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: PWM_Source_Clk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    PWM_Source_Clk_PHASE = clkPhase & PWM_Source_Clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: PWM_Source_Clk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return PWM_Source_Clk_PHASE & PWM_Source_Clk_PHASE_MASK;
}
